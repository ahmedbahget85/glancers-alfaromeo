...
foreach ( glob( ELATED_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( etienne_elated_is_customizer_item_enabled( $module_load, 'etienne_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'etienne_elated_get_icon_pack_collections' ) ) {
	function etienne_elated_get_icon_pack_collections() {
		$options = apply_filters( 'etienne_elated_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'etienne_elated_get_icon_pack_collections' );
}

if ( ! function_exists( 'etienne_elated_get_icon_pack_options' ) ) {
	function etienne_elated_get_icon_pack_options() {
		$options = apply_filters( 'etienne_elated_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'etienne_elated_get_icon_pack_options' );
}

if ( ! function_exists( 'etienne_elated_get_icon_pack_social_options' ) ) {
	function etienne_elated_get_icon_pack_social_options() {
		$options = apply_filters( 'etienne_elated_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'etienne_elated_get_icon_pack_social_options' );
}

/*
 * Class: EtienneElatedClassIconCollections
 */
class EtienneElatedClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = etienne_elated_get_icon_pack_collections();
		$this->iconPackOptions       = etienne_elated_get_icon_pack_options();
		$this->iconPackSocialOptions = etienne_elated_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = etienne_elated_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'etienne' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'etienne' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'etienne' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'etienne' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'etienne' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'etienne' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'etienne' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'etienne' )       => 'fa-lg',
			esc_html__( 'Small', 'etienne' )      => 'fa-2x',
			esc_html__( 'Medium', 'etienne' )     => 'fa-3x',
			esc_html__( 'Large', 'etienne' )      => 'fa-4x',
			esc_html__( 'Very Large', 'etienne' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iEtienneElatedInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'etienne' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'etienne' ) ] = '';
