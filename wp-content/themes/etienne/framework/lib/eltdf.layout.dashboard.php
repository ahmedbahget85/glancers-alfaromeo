...
/*
   Class: EtienneElatedClassDashboardForm
   A class that initializes Elated Dashboard Container
*/

class EtienneElatedClassDashboardForm implements iEtienneElatedInterfaceLayoutNode, iEtienneElatedInterfaceRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
	...
	public function render( $factory ) {
		...
		<form method="<?php echo esc_attr( $this->form_method ); ?>" id="<?php echo esc_attr( $this->form_id ); ?>" class="<?php echo esc_attr( $action_class ) ?>" data-action="<?php echo esc_attr( $this->form_action ); ?>">
			...
			if ( etienne_elated_core_plugin_installed() ) {
				echo etienne_elated_get_button_html( array(
					'html_type'    => 'button',
...
/*
   Class: EtienneElatedClassDashboardGroup
   A class that initializes EtienneElatedClass Group Field
*/
class EtienneElatedClassDashboardGroup implements iEtienneElatedInterfaceLayoutNode, iEtienneElatedInterfaceRender {
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: EtienneElatedClassDashboardTitle
   A class that initializes Dashboard Title
*/
class EtienneElatedClassDashboardTitle implements iEtienneElatedInterfaceRender {
	private $name;
	private $title;
	private $args = array();
...
/*
   Class: EtienneElatedClassDashboardField
   A class that initializes EtienneElatedClass Front Field
*/
class EtienneElatedClassDashboardField implements iEtienneElatedInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class EtienneElatedClassDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class EtienneElatedClassDashboardFieldText extends EtienneElatedClassDashboardFieldType {
...
class EtienneElatedClassDashboardFieldTextArea extends EtienneElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class EtienneElatedClassDashboardFieldImage extends EtienneElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
			...
			<div class="eltdf-dashboard-gallery-holder">
				<ul class="eltdf-dashboard-gallery-images-holder">
					<?php if ( isset( $value_html ) ) {
						echo elaine_edge_display_content_output($value_html);
					} ?>
				</ul>
				<div class="eltdf-dashboard-gallery-uploader">
					<?php
					if ( etienne_membership_theme_installed() ) {
						echo etienne_elated_get_button_html( array(
							'text'         => esc_html__( 'Upload', 'etienne' ),
							'custom_class' => 'eltdf-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="eltdf-btn eltdf-btn-medium eltdf-btn-solid eltdf-dashboard-gallery-upload"><span class="eltdf-btn-text">' . esc_html__( 'Upload', 'etienne' ) . '</span></a>';
					} ?>
					...
					<?php if ( $value !== '' && $value !== false ) { ?>
						<button class="eltdf-btn eltdf-btn-solid eltdf-dashboard-remove-image"><?php esc_html_e( 'Remove Media', 'etienne' ); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class EtienneElatedClassDashboardFieldGallery extends EtienneElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class EtienneElatedClassDashboardFieldSelect extends EtienneElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class EtienneElatedClassDashboardFieldDate extends EtienneElatedClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class EtienneElatedClassDashboardFieldIcon extends EtienneElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
...
class EtienneElatedClassDashboardFieldColor extends EtienneElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class EtienneElatedClassDashboardFieldCheckBoxGroup extends EtienneElatedClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class EtienneElatedClassDashboardFieldAddress extends EtienneElatedClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="eltdf-dashboard-field-row">
				<div class="eltdf-dashboard-item col-lg-<?php echo esc_attr( $col_width ); ?>">
					<div class="eltdf-dashboard-input-holder">
						...
						<a class="eltdf-reset-marker eltdf-hide-field" href="#"><?php esc_html_e( 'Reset Marker', 'etienne' ); ?></a>
...
class EtienneElatedClassDashboardRepeater implements iEtienneElatedInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'etienne' );
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="eltdf-dashboard-repeater-table-heading">
					<div class="eltdf-dashboard-repeater-fields-holder">
						<div class="eltdf-dashboard-repeater-table-cell eltdf-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'etienne' ) ?></div>
						...
						<div class="eltdf-dashboard-repeater-table-cell eltdf-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'etienne' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class EtienneElatedClassDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new EtienneElatedClassDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'textarea':
				$field = new EtienneElatedClassDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'date':
				$field = new EtienneElatedClassDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new EtienneElatedClassDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'gallery':
				$field = new EtienneElatedClassDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new EtienneElatedClassDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new EtienneElatedClassDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new EtienneElatedClassDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'checkboxgroup':
				$field = new EtienneElatedClassDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'address':
				$field = new EtienneElatedClassDashboardFieldAddress();
