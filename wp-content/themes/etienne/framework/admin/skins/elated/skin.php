...
class ElatedSkin extends EtienneElatedClassSkinAbstract {
	
    /**
     * Skin constructor. Hooks to etienne_elated_admin_scripts_init and etienne_elated_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'elated';
		
		// Admin register scripts and styles
		add_action( 'etienne_elated_action_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'etienne_elated_action_admin_scripts_init', array( $this, 'registerScripts' ) );
		
		// Admin enqueue scripts and styles
		add_action( 'etienne_elated_action_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'etienne_elated_action_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );
		
		// Admin enqueue meta box scripts and styles
		add_action( 'etienne_elated_action_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'etienne_elated_action_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
		
		// Change options map order for this skin
		add_filter( 'etienne_elated_filter_options_map_position', array( $this, 'setOptionsMapPosition' ), 10, 2 );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			etienne_elated_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
	        etienne_elated_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    = etienne_elated_get_admin_tab();
        $active_page = $etienne_elated_global_Framework->eltdOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see EtienneElatedClassSkinAbstract::loadTemplatePart()
     */
	 ...
	 *
	 * @param EtienneElatedClassAdminPage $page current page to load
	 * @see EtienneElatedClassSkinAbstract::loadTemplatePart()
	 */
